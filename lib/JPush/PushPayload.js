...
function ios (alert, sound, badge, contentAvailable, extras, category, mutableContent) {
  // jiji - 16.9.19 由于ios10推送调整，修改alert可接受json对象
  // if (alert != null) {
  //   if (typeof alert !== 'string') {
  ...
  //jiji - 16.9.19 - 适配iOS10的extension，可修改推送内容
  if (mutableContent != null) {
    if (typeof mutableContent !== 'boolean') {
      throw new JError.InvalidArgumentError('Invalid ios.mutableContent, it can only be set to the Boolean')
    }
    if (mutableContent) {
      ios['mutable-content'] = 1
    }
  }
